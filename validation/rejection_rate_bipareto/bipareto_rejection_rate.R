...
#
# usage: 
#   ./bipareto_rejection_rate.R '../lib/outskewer/'
...
biparetoRate.df <- Simulation(generate.bipareto)
ExportData(biparetoRate.df, 'bipareto_rejection_rate')
save.image('bipareto_rejection_rate')

qBipareto <- PlotRate(biparetoRate.df) 
qBipareto <- qBipareto + scale_x_log(breaks=c(10,100, 1000), labels=c(10,100,1000))
qBipareto <- qBipareto + scale_y_log("rejection rate", breaks=c(0.001,0.01,0.1,1), labels=c(0.001,0.01,0.1,1))
ExportPlot(qBipareto, 'bipareto_rejection_rate', 4,3)
