...
generate.gaussian <- function(n) { return(rnorm(n, mean=0, sd=1)) }
...
gaussianRate.df <- Simulation(generate.gaussian, times=1000)
ExportData(gaussianRate.df, 'false_positive_norm')
save.image('false_positive_norm')

qGaussian <- PlotRate(gaussianRate.df)
qGaussian$continuous <- qGaussian$continuous + scale_y_continuous("outlier rate", breaks=c(0, 0.01, 0.02, 0.03))
qGaussian$log <- qGaussian$log + scale_y_log10("outlier rate", breaks=c(0.03, 0, 0.01, 0.001, 0.0001, 0.00001), labels=c(0.03, 0, 0.01, 0.001, '0.0001', '0.00001'))
ExportPlot(qGaussian$continuous, 'false_positive_norm', 4.5, 1.5)
ExportPlot(qGaussian$log, 'false_positive_norm_log', 4, 3)
