...
library(actuar)
generate.bipareto <- function(n) {
    n1 <- floor(n/2)
    x1 <- actuar::rpareto(n1, 6 ,2)
    x2 <- - actuar::rpareto(n - n1, 6 ,2)
    return(c(x1, x2))
}
...
biparetoRate.df <- Simulation(generate.bipareto, times=1000)
ExportData(biparetoRate.df, 'false_positive_bipareto')
save.image('false_positive_bipareto')

qBipareto <- PlotRate(biparetoRate.df)
qBipareto$continuous <- qBipareto$continuous + scale_y_continuous("outlier rate", breaks=c(0, 0.01, 0.03, 0.05))
qBipareto$log <- qBipareto$log + scale_y_log10("outlier rate", breaks=c(0.05, 0, 0.01, 0.001, 0.0001, 0.00001), labels=c(0.05, 0, 0.01, 0.001, '0.0001', '0.00001'))
ExportPlot(qBipareto$continuous, 'false_positive_bipareto', 4.5, 1.5)
ExportPlot(qBipareto$log, 'false_positive_bipareto_log', 4, 3)
