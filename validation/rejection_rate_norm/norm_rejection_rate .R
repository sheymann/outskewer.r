...
#
# usage: 
#   ./norm_rejection_rate.R '../lib/outskewer/'
...
gaussianRate.df <- Simulation(generate.gaussian)
#gaussianRate.df <- subset(gaussianRate.df, gaussianRate.df$reject > 0)
ExportData(gaussianRate.df, 'norm_rejection_rate')
save.image('norm_rejection_rate')

qGaussian <- PlotRate(gaussianRate.df)
qGaussian <- qGaussian + scale_x_log(breaks=c(10,100, 1000), labels=c(10,100,1000))
qGaussian <- qGaussian + scale_y_log("rejection rate", breaks=c(0.001,0.01,0.1,1), labels=c(0.001,0.01,0.1,1))
ExportPlot(qGaussian, 'norm_rejection_rate', 4,3)
